-- =====================================================
-- FIX RLS POLICIES FOR DATA FETCHING ISSUES
-- =====================================================
...
CREATE POLICY "Organizers can update own events or admins can update any" ON events
    FOR UPDATE USING (
        auth.uid() = organizer_id OR 
        is_admin()
    );
...
-- BOOKINGS POLICIES
CREATE POLICY "Users can view own bookings, organizers can view event bookings, admins can view all" ON bookings
    FOR SELECT USING (
        user_id = auth.uid() OR
        EXISTS (
...
CREATE POLICY "Users can update own bookings or admins can update any" ON bookings
    FOR UPDATE USING (
        auth.uid() = user_id OR 
        is_admin()
    );
...
CREATE POLICY "Users, organizers, and admins can view relevant payments" ON payments
    FOR SELECT USING (
...
CREATE POLICY "Users and admins can update payments" ON payments
    FOR UPDATE USING (
        booking_id IN (
            SELECT id FROM bookings WHERE user_id = auth.uid()
        ) OR
...
CREATE POLICY "Users, organizers, and admins can view relevant payment logs" ON payment_logs
    FOR SELECT USING (
        booking_id IN (
            SELECT id FROM bookings WHERE user_id = auth.uid()
        ) OR
...
-- =====================================================
-- STEP 4: ADD MISSING TABLES IF THEY DON'T EXIST
-- =====================================================
...
-- Tickets policies
CREATE POLICY "Users, organizers, and admins can view relevant tickets" ON tickets
    FOR SELECT USING (
        booking_id IN (
            SELECT id FROM bookings WHERE user_id = auth.uid()
        ) OR
...
CREATE POLICY "Organizers and admins can update tickets" ON tickets
    FOR UPDATE USING (
        booking_id IN (
            SELECT b.id FROM bookings b
            JOIN events e ON e.id = b.event_id
...
-- This should return events for testing
SELECT 
