...
export default function SimplifiedAnalyticsPage() {
  ...
  useEffect(() => {
    ...
    loadAnalytics()
  }, [authLoading, isAdmin, selectedCategory])
  ...
  const loadAnalytics = async () => {
    try {
      ...
      // Load events with their categories
      ...
      // Load all tickets with their event info
      ...
      // Load predefined ticket templates to get the actual ticket types and levels
      const { data: predefinedData } = await supabase
        .from('predefined_tickets')
        .select('event_id, ticket_type, ticket_level, name')
      ...
      eventsData?.forEach(event => {
        ...
        eventTickets.forEach(ticket => {
          ...
          // First check predefined templates for exact match
          const predefined = predefinedData?.find(p => 
            p.event_id === event.id && p.ticket_type === ticketType
          )
          
          if (predefined?.ticket_level) {
            // Use the level from predefined template
            level = predefined.ticket_level as 'Gold' | 'Silver' | 'Bronze'
          } else {
            // Fallback to analyzing ticket type name
            const typeLower = ticketType.toLowerCase()
            if (typeLower.includes('gold') || typeLower.includes('vip') || typeLower.includes('platinum')) {
              level = 'Gold'
            } else if (typeLower.includes('silver') || typeLower.includes('premium')) {
              level = 'Silver'
            } else if (typeLower.includes('bronze') || typeLower.includes('standard') || typeLower.includes('general')) {
              level = 'Bronze'
            }
          }
