...
export async function POST(request: NextRequest) {
  try {
    ...
    let imageDataToStore = null
    let imageType = null
    let processedEventData = { ...eventData }
    
    if (eventData.image_url && eventData.image_url.startsWith('data:')) {
      // Extract base64 data from data URL
      const matches = eventData.image_url.match(/^data:(.+);base64,(.+)$/)
      if (matches) {
        imageType = matches[1]
        imageDataToStore = matches[2]
        // Remove the data URL from event data (we'll store it separately)
        processedEventData.image_url = null
      }
    }
    ...
    // Store image in database if base64 data is provided
    if (imageDataToStore && event) {
      console.log('Storing image in database for event:', event.id)
      
      const { data: imageResult, error: imageError } = await supabase
        .rpc('store_event_image', {
          p_event_id: event.id,
          p_image_data: imageDataToStore,
      ...
      if (imageError) {
        ...
        console.log('Image stored successfully:', imageResult)
      }
    }
